...
namespace BnL.GDBSubstringReplacerInTextfields.UI
{
    internal static class ConsoleUi
    {
        public static void Banner()
        {
            try
            {
                AnsiConsole.Write(new FigletText("GDB URL Replacer").Color(Color.Green));
            }
            catch
            {
                AnsiConsole.MarkupLine("[bold green]GDB URL Replacer[/]");
            }
        }

        public static void ShowScanIntro(string startDirectory, string oldSubstring, string newSubstring)
        {
            var panel = new Panel(new Markup($"[grey]Scanning[/] [bold]{Escape(startDirectory)}[/]\n[grey]Replace[/] [yellow]{Escape(oldSubstring)}[/] [grey]→[/] [green]{Escape(newSubstring)}[/]"))
            {
        ...
        {
            return AnsiConsole.Confirm("Proceed with replacing URLs?", defaultValue: false);
        }
