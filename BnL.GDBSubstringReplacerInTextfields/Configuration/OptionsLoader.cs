...
namespace BnL.GDBSubstringReplacerInTextfields.Configuration
{
    internal static class OptionsLoader
    {
        public static ToolOptions Load(string[] args)
        {
            ...
            {
                ["--start"] = "GdbSubstringReplacer:StartDirectory",
                ["-s"] = "GdbSubstringReplacer:StartDirectory"
            };

            var configuration = new ConfigurationBuilder()
                .SetBasePath(AppContext.BaseDirectory)
                .AddJsonFile("appsettings.json", optional: false)
                .AddEnvironmentVariables("GDBSUBSTRINGREPLACER_")
                .AddCommandLine(args, switchMappings)
                .Build();

            var options = new ToolOptions();
            configuration.GetSection("GdbSubstringReplacer").Bind(options);
            ...
            {
                throw new InvalidOperationException("Configure GdbSubstringReplacer:StartDirectory in appsettings.json or pass --start.");
            }
