...
public partial class OgctLayer : IOgctLayer
{
    ...
    public long CopyFeatures(IOgctLayer targetLayer, bool generateNewFids = false, Action<int> reportProgressPercentage = null)
    {
        ...
        while ((sourceFeature = _layer.GetNextFeature()) != null)
        {
            ...
            {
                newFeature.CreateTableRecordFromOther(fields, sourceFields, sourceFeature, LayerDetails.FidColumnName, targetLayer.LayerDetails.FidColumnName  );

            }
            else
            {
                newFeature.CreateFromOther(fields, sourceFields, sourceFeature, LayerDetails.FidColumnName, targetLayer.LayerDetails.FidColumnName  );
    ...
    private void CloneFieldSchema(IOgctLayer targetLayer)
    {
        ...
        for (int j = 0; j < _layer.GetLayerDefn().GetFieldCount(); j++)  // create fields in outputLayer
        {
            ...
            if (name == targetOgctLayer.LayerDetails.FidColumnName)
            {
                continue; // ignore FID-field, its automatically generated by the database
            }
